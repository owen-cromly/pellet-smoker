...
for i=1:N
    s=@(v)v*(0.8+0.4*rand);
    p_i=p; p_i.Cf=s(p.Cf); p_i.Cc=s(p.Cc); p_i.kf=s(p.kf); p_i.kfa=s(p.kfa); p_i.kca=s(p.kca);
    [A_i,B_i,C_i,~]=smoker_lin(p_i, op); eq_i=smoker_eq(p_i, op); L_i=observer_gain(p_i, op, 5*[-0.02 -0.03]);

    dt=0.5; T=1200; TT=(0:dt:T)'; xabs=eq_i.xe; zhat=zeros(3,1); xI=0; uabs=eq_i.ue;
    ylog=zeros(size(TT)); uplog=ylog; du_max=0.03; u_floor=[max(0.10,eq_i.ue(1)); max(0.35,eq_i.ue(2))];

    for k=1:numel(TT)
        y = xabs(2); ydev=y - eq_i.y0;
        du_des = -K*zhat - Ki*xI;
